...
public class TweetNaCl {
    ...
    public static boolean crypto_sign_verify(byte[] msg, byte[] sig, byte[] publicKey) {
        if (sig.length != SIGNATURE_SIZE_BYTES)
            throw new IllegalStateException("Illegal sig length: " + sig.length);
        if (publicKey.length != SIGN_PUBLIC_KEY_BYTES)
            throw new IllegalStateException("Illegal publicKey length: " + publicKey.length);
        byte[] sm = new byte[SIGNATURE_SIZE_BYTES + msg.length];
        byte[] m = new byte[SIGNATURE_SIZE_BYTES + msg.length];
        for (int i = 0; i < SIGNATURE_SIZE_BYTES; i++) sm[i] = sig[i];
        for (int i = 0; i < msg.length; i++) sm[i + SIGNATURE_SIZE_BYTES] = msg[i];
